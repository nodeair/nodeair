...
const steps = {
  ...
  '从插件列表中去掉【run】插件': async function () {
    const { app } = this;
  ...
  '初始化app实例': async function () {
    const { app } = this;
    await app.init();
  },
  '移除所有表': async function() {
    const { app } = this;
    const { db } = app;
    await db.model.dropAll();
  },
  '如果数据库类型是 sqlite 则删除对应的文件': async function() {
    const { app } = this;
  ...
  '修改【install】插件的启用状态': async function () {
    const { app } = this;
    const { plugin } = app;
